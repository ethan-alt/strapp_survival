...
parameters {
  ...
  vector[p]             c0;           // gen strapp param
  real<lower=0>         c0_sd;        // SD for gen strapp
}
...
transformed parameters {
  ...
  vector[p] beta_hist = sigma * invsqrt_X0tX0 * ( sqrt_fisher_promotiontime(X0, beta, intercept ) * beta - c0 );
}
